...
kb = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton(text="🔧 Связь с тех. поддержкой"), KeyboardButton(text="🏁 Старт")]
...
async def support_handler(message: types.Message):
    await message.answer(
        "Если у тебя возникли проблемы, напиши на email:  xcommander67@gmail.com или обратись через /start для получения помощи."
    )
...
async def start_message_handler(message: types.Message):
    await message.answer("👋 Привет! Я бот, который решает задачи с помощью ИИ. Нажми на кнопку 'Решить задачу', чтобы начать!", reply_markup=kb)
